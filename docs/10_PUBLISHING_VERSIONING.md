...
3. ~~**`navigation_items` n'est pas isolée** (`nav_items_public_read`, 022/026) : un libellé de menu modifié est public immédiatement, alors que le contrôle n°2 du §24 valide la navigation *avant* publication.~~ **PÉRIMÉ.** Le 2026-09-19, la navigation a été réexaminée : le site public **ne rend pas** `navigation_items` du tout. `PublicNav.tsx` et `Footer.tsx` portent des listes **écrites en dur**, et **aucun écran d'administration ne touche la navigation**. Le point décrivait donc un risque qui n'existe pas — mais il en révélait un autre : les contrôles n°2 et n°6 du §24 validaient 13 liens que **personne ne voyait**. Décision : ces deux contrôles sont déclarés **non vérifiés** dans le rapport au lieu d'afficher un vert mensonger (`PUBLICATION_CHECKS_NOT_VERIFIED`). Les ancres réellement servies restent vérifiées au développement par `npm run verify:anchors`.
   > ⚠️ **CORRECTION DU 2026-09-20 (revue indépendante, I-1).** La première rédaction de ce point affirmait « mesuré : `fetchSiteNavigation` n'est appelé que par `publishing.ts` ». **C'est faux**, et c'était faux au moment même où c'était écrit : la décision ci-dessus venait précisément de retirer ce dernier appel. Mesure : `fetchSiteNavigation`, `createNavigationItem` et `updateNavigationItem` n'ont **aucun appelant** dans `src/` — leur seule mention est leur définition et un ré-export. Le lot intitulé « retirer les affirmations que le dépôt contredit » en avait donc introduit une. Leçon retenue : une affirmation « mesuré » doit porter sa mesure et être re-mesurée au commit qui la publie.
