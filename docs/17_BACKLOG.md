...
## B-S2. Filet lot1 contr�le C ? r�f�rence `0528c544` obsol?te apr?s J2/J4/dispositions

**Constat� le** : 2026-09-24 (triage audit syst?me).

**Mesure** : `npm run verify:lot1` �choue au contr�le C (non-r�gression HTML)
...
`0528c544`. Les �carts sont **attendus** apr?s le travail J2/J4 et les
dispositions ? ce n'est pas une r�gression accidentelle ? ? corriger ? en
r��crivant le site pour coller ? l'ancien HTML.

**Ce que disent d�j? les docs** :
- `scripts/verify-lot1.mjs` et `AGENTS.md` �15 **�pinglent** `PRE_CMS_WIRING_REF
  = 0528c544` ; une surcharge `BASE_REF` sans `GLIFE_ALLOW_REF_OVERRIDE=1` est
  **refus�e** (garde-fou anti faux-vert, revue 2026-09-20).
- Le seul changement de SHA document� est la r��criture d'historique
...
- `docs/18_DISPOSITIONS_DE_BLOC.md` �6.1 exige encore que la disposition par
  d�faut rende comme `0528c544` ; cette exigence n'a **pas** �t� r�vis�e apr?s
  les lots UX intentionnels.

**D�cision propri�taire requise** (ne pas inventer ici) :
1. **Rebaser** `PRE_CMS_WIRING_REF` sur un commit post-J2/J4/dispositions, et
   mettre ? jour `AGENTS.md` + `docs/18` en m?me temps ; ou
2. **Restreindre** le contr�le C (ex. disposition par d�faut seulement /
   sections encore fig�es) ; ou
3. **Marquer C comme dette connue** / le rendre informatif jusqu'? arbitrage.

**Statut** : dette document�e, **aucune modification de `BASE_REF`** dans ce
triage. Le rouge du contr�le C reste un signal honn?te tant que l'arbitrage
n'est pas rendu.
