...
m' }}>
        ...
        <nav className="desktop-nav" aria-label="Navigation principale" style={{ display: 'flex', gap: '28px', alignItems: 'center' }}>
          {links.map(([l, id]) => (
            <a key={l} href={`#${id}`} aria-current={active === id ? 'true' : undefined}
              ...
              onMouseLeave={e => (e.currentTarget.style.color = active === id ? t.accent : t.text)}
            >{l}</a>
          ))}
        </nav>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <a href="#contact" className="desktop-nav" aria-label="Réserver une table" style={{
            ...
            boxShadow: softShadowSm(t), transition: 'transform 0.2s',
          }}
      ...
      <AnimatePresence>
        {drawerOpen && isMobile && (
          <motion.nav initial={{ x: '100%', opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: '100%', opacity: 0 }}
            transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} aria-label="Menu mobile"
            style={{
            ...
            <button aria-label="Fermer" onClick={() => setDrawerOpen(false)}
              style={{ position: 'absolute', top: '20px', right: '20px', width: '36px', height: '36px', borderRadius: '10px', background: t.surfaceAlt, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: t.heading, fontSize: '20px' }}>✕</button>
            {links.map(([l, id], i) => (
...
0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <a href="#contact" onClick={() => setDrawerOpen(false)}
              style={{ marginTop: '16px', textAlign: 'center', fontSize: '15px', fontWeight: 600, color: '#fff', background: t.primary, padding: '14px', borderRadius: '100px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: softShadowSm(t) }}>
