...
export function Team({ content: cms, variant }: Partial<SectionComponentProps> = {}) {
              ...
              <Reveal key={m.name} delay={(i % 4) * 0.06}>
                <OrganicCard hover style={{ padding: 0, overflow: 'hidden', display: 'flex', alignItems: 'stretch' }}>
                  <div style={{
                    ...
                    {!teamImgs[i] && (
                      <span style={{ fontFamily: 'var(--f-heading)', fontSize: '40px', fontWeight: 700, color: m.color, opacity: 0.35 }} aria-hidden="true">{m.name.charAt(0)}</span>
                    )}
                  </div>
