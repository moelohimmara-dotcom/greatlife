...
export function Team({ content: cms, variant, preview }: Partial<SectionComponentProps> = {}) {
  ...
  if (disposition === 'list') {
    return (
      <section className="section-pad" data-disposition="list" style={{ padding: '100px 24px', background: t.surfaceAlt }}>
              ...
              <Reveal key={m.name} delay={(i % 4) * 0.06}>
                <OrganicCard hover style={{ padding: 0, overflow: 'hidden', display: 'flex', alignItems: 'stretch' }}>
                  ...
                  <div style={{ padding: '18px 22px' }}>
                    <h4 style={{ fontFamily: 'var(--font-heading, var(--f-heading))', color: t.heading, fontSize: '18px', fontWeight: 700, margin: '0 0 3px', letterSpacing: '-0.02em' }}>{m.name}</h4>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: m.color, marginBottom: '8px' }}>{m.role}</div>
                    <p style={{ fontSize: '13.5px', color: t.muted, lineHeight: 1.55, margin: 0 }}><InlineHtml as="span" html={m.desc ?? ''} /></p>
                  </div>
                </OrganicCard>
              </Reveal>
              )
