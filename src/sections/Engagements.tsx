...
export function Engagements({ content: cms, variant, preview }: Partial<SectionComponentProps> = {}) {
          ...
          {items.map(([ic, itemTitle, desc], i) => (
            <Reveal key={i} delay={(i % 3) * 0.06}>
              <div style={{
                ...
                <div>
                  <h4 style={{ fontFamily: 'var(--font-heading, var(--f-heading))', color: t.heading, fontSize: '18px', fontWeight: 700, margin: '0 0 6px', letterSpacing: '-0.02em' }}>{itemTitle}</h4>
                  <p style={{ fontSize: '14px', color: t.muted, lineHeight: 1.55, margin: 0 }}><InlineHtml as="span" html={desc} /></p>
                </div>
              </div>
            </Reveal>
          ))}
        ...
        {items.map(([ic, itemTitle, desc], i) => (
          <Reveal key={i} delay={(i % 3) * 0.06}>
            <OrganicCard hover style={{ padding: '28px' }}>
              ...
              <h4 style={{ fontFamily: 'var(--font-heading, var(--f-heading))', color: t.heading, fontSize: '18px', fontWeight: 700, margin: '0 0 8px', letterSpacing: '-0.02em' }}>{itemTitle}</h4>
              <p style={{ fontSize: '14px', color: t.muted, lineHeight: 1.55, margin: 0 }}><InlineHtml as="span" html={desc} /></p>
            </OrganicCard>
          </Reveal>
        ))}
